...
passport.use(new LocalStrategy(function(username, password, done) { //This password is called from password.authenticate, from user routes.
  ...
  .exec(function(err, user) {  
  	if(err) return done(500).send({err: "Server has issues."});
  	if(!user) return done(400).send({err: "User does not exist"});
  	if(!user.checkPassword(password)) return done(400).send({err: "Invalid username and password combination."});
